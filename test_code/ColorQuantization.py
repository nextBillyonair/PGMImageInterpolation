...
class ColorQuantization(object):
    ...
    def __init__(self, n_colors=8, algo='mini_batch_kmeans'):
    ...
    def _init_model(self, algo, n_colors):
        if algo == 'mini_batch_kmeans':
            self.model = MiniBatchKMeans(n_clusters=n_colors, compute_labels=False)
        elif algo == 'kmeans':
            self.model = KMeans(n_clusters=n_colors)
        elif algo == 'gmm':
            self.model = GaussianMixture(n_components=n_colors)
        else:
            raise ValueError(f"ERROR: {algo} not acceptable\nPlease choose: ['mini_batch_kmeans', 'kmeans', 'gmm']")
    ...
    def get_centers(self):
        if self.algo in ['mini_batch_kmeans', 'kmeans']:
